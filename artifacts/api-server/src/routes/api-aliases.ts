...
// /api/futures/orderbook → /api/admin/inmem-engine/orderbook/:symbol
// (public read alias — no admin required for display)
router.get("/futures/orderbook", (req, res, next) => {
  const symbol = (req.query.pair as string ?? req.query.symbol as string ?? "BTCUSDT").toUpperCase();
  req.url = `/admin/inmem-engine/orderbook/${symbol}`;
  next("router");
});
