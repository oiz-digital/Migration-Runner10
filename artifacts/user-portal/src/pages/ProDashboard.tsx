...
/** Minimum quote-volume (USDT) to be considered a liquid market */
const MIN_LIQUID_VOL = 5_000;
...
export default function ProDashboard() {
  ...
  const coinMeta = useMemo(() => {
    ...
    for (const c of list) {
      if (c?.currency) map.set(String(c.currency), { name: c.name ?? c.currency, icon: c.icon ?? null });
    }
    return map;
  }, [currencyList]);

  /* ── derived: USDT coins with live price ────────────────── */
  const coins: Coin[] = useMemo(() => {
    if (!Array.isArray(marketsRaw)) return [];
    return marketsRaw
      .filter((m) => m && typeof m.symbol === "string" && m.symbol.endsWith("/USDT"))
      .map((m) => {
        const sym = String(m.currency ?? m.symbol.split("/")[0] ?? "");
  ...
  /* ── movers: liquid coins only, sorted by |change| ──────── */
  const movers = useMemo(
    () =>
      [...coins]
        .filter((c) => c.volume24h >= MIN_LIQUID_VOL)
    ...
    <div className="container mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-5">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          >
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {movers.length === 0 ? (
                ...
                return (
                  <Link key={c.symbol} href={`/trade/${encodeSymbol(c.symbol + "/USDT")}`}
                    ...
                    <div className="text-[11px] text-muted-foreground font-mono mt-1">
                      {px.toLocaleString(undefined, { maximumFractionDigits: px < 1 ? 6 : 2 })} USDT
                    </div>
          ...
          <SectionCard
            title="Top markets"
            description="Highest-volume USDT pairs — click any row to trade."
            ...
            <div className="space-y-0.5">
              ...
              <div className="flex items-center justify-between gap-2 px-2 py-1 text-[10px] text-muted-foreground uppercase tracking-wide">
                ...
                <div className="flex items-center gap-6">
                  <span className="hidden sm:block w-20 text-right">Volume (USDT)</span>
                    ...
                    <div className="flex items-center gap-4 sm:gap-6 shrink-0">
                      ...
                      <div className="w-20 text-right">
                        <div className="font-mono text-sm font-bold">
                          {px.toLocaleString(undefined, { maximumFractionDigits: px < 1 ? 6 : 2 })}
                        </div>
                  ...
                  return (
                    <div key={a.id} className="flex items-center justify-between gap-2 text-xs">
                      ...
                      <span className="text-muted-foreground font-mono text-[11px]">
                        {Number(a.targetPrice).toLocaleString(undefined, { maximumFractionDigits: 4 })} USDT
                      </span>
                    </div>
                  );
                })}
