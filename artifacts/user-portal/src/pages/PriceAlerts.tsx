...
export default function PriceAlerts() {
  ...
  const disableMut = useMutation({
    mutationFn: async (id: number) => {
      const r = await fetch(`/api/price-alerts/${id}/disable`, { method: "PATCH", credentials: "include" });
          ...
          {active.length > 0 && (
            <SectionCard title={`Active (${active.length})`} icon={Bell} padded={false}>
              <div className="divide-y divide-border/40">
                {active.map(a => <AlertRow key={a.id} alert={a} onDelete={() => deleteMut.mutate(a.id)} onDisable={() => disableMut.mutate(a.id)} />)}
              </div>
            </SectionCard>
          )}
          {triggered.length > 0 && (
            <SectionCard title={`Triggered (${triggered.length})`} icon={TrendingUp} padded={false}>
              <div className="divide-y divide-border/40">
                {triggered.map(a => <AlertRow key={a.id} alert={a} onDelete={() => deleteMut.mutate(a.id)} />)}
              </div>
            </SectionCard>
          )}
          {disabled.length > 0 && (
            <SectionCard title={`Disabled (${disabled.length})`} icon={X} padded={false}>
              <div className="divide-y divide-border/40">
                {disabled.map(a => <AlertRow key={a.id} alert={a} onDelete={() => deleteMut.mutate(a.id)} />)}
              </div>
            </SectionCard>
          )}
        </div>
