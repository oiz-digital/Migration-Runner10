          ...
          <div className="w-1/2 lg:w-full lg:h-1/2 flex flex-col border-r lg:border-r-0 lg:border-b border-border min-h-0">
            ...
            <div className="flex-1 overflow-auto px-2 py-1 text-xs font-mono">
              ...
              {(() => {
                ...
                for (const [, q] of asks) { cumAsk += q; cumAsks.push(cumAsk); }
                return asks.slice().reverse().map(([px, qty], i) => {
                  const cumIdx = bookRows - 1 - i;
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="relative text-destructive tabular-nums text-left text-[11px]">{fmtNum(px, quote === "INR" ? 2 : 4)}</span>
                      <span className="relative text-right tabular-nums text-[11px]">{fmtNum(qty, 4)}</span>
                      <span className="relative text-right tabular-nums text-muted-foreground/70 text-[10px]">{fmtNum(cumAsks[cumIdx] ?? 0, 2)}</span>
                    </button>
                  );
                });
              })()}
                ...
                return bids.map(([px, qty], i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="relative text-success tabular-nums text-left text-[11px]">{fmtNum(px, quote === "INR" ? 2 : 4)}</span>
                      <span className="relative text-right tabular-nums text-[11px]">{fmtNum(qty, 4)}</span>
                      <span className="relative text-right tabular-nums text-muted-foreground/70 text-[10px]">{fmtNum(cumBid, 2)}</span>
                    </button>
                  );
                });
              })()}
